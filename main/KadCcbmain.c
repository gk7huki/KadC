/**
 * A modified version of KadCmain.c that uses
 * for searches the callback interface to report
 * the found matches immediately
 */
#include <pthread.h>
#include <stdio.h>
#include <string.h>
...
static int hit_to_log_cb(KadCdictionary *pkd, void *context) {
	KadC_log("Found: (context %s)", context);
	...
	for(;;) {
		...
		if(strcmp(par[0], "s") == 0 || strcmp(par[0], "search") == 0) {
...

			{
				...
				fpar.hit_callback = hit_to_log_cb;
				fpar.hit_callback_context = "Test";
#ifdef SEARCH_COLLECT
				fpar.hit_callback_mode = KADC_COLLECT_HITS;
#endif
				resdictrbt = KadC_find2(pkcc, index, &fpar);
			}
#ifdef SEARCH_COLLECT
